...
def compute_phi_index_obs_dtw(data, phi ,mean_weight, sigma_weight, use_KF= True, use_lookahead= True):
    ...
    if use_KF:
        ...
        if use_lookahead:
            ...
            inferred_traj, _, _ = reconstruct_seg_sg(data, mean_weight, sigma_weight, phi_data,
                                                                       plot=True)
...
if __name__ == "__main__":
    ...
    for i in range(len(test_x)): # all data for testing
        ...
        _, inferred_traj = compute_phi_index_obs_dtw(obs_traj,big_psi,mean_ent_ww, sigma_ent_ww, use_KF=True, use_lookahead=False)
        ...
        plt.plot(inferred_traj[0:len(inferred_traj): D], inferred_traj[1: len(inferred_traj): D], 'bo', label='inferred all')
        ...
        inferred_data_len = int(len(inferred_traj)/D)
        inferred_x = inferred_traj[0:len(inferred_traj):D]
        inferred_y = inferred_traj[1:len(inferred_traj):D]
        inferred_psi = inferred_traj[2:len(inferred_traj):D]
        inferred_vx = inferred_traj[3:len(inferred_traj):D]
        inferred_vy = inferred_traj[4:len(inferred_traj):D]
        # primitive_traj= big_psi* mean_pos_ww
        # primitive_x = primitive_traj[0:current_segment.shape[0]:D]
        # primitive_y = primitive_traj[0:current_segment.shape[0]:D]
        observation_x = test_x[i][0:inferred_data_len]
        observation_y = test_y[i][0:inferred_data_len]
        observation_psi = test_psi[i][0:inferred_data_len]
        observation_vx = test_vx[i][0:inferred_data_len]
        observation_vy = test_vy[i][0:inferred_data_len]
        calc_accuracy = True
        if calc_accuracy:
        ...
        obs_traj = []
    ##################################looking at the first 20 obsevation for phi and trajectory
    # obs_traj = []
    # obs_data_len = 20
    # for i in range(len(test_x)):  # all data for testing
    #     for j in range(obs_data_len):
    ...
    #         plt.plot(observation_x, observation_y, "--", color=obv_colors[current_cluster],
    #                  label="Observation" + str(current_cluster), linewidth=2.0)
    #         plt.plot(inferred_x, inferred_y, ".", color=inf_colors[current_cluster],
    #                  label="Inferred" + str(current_cluster), linewidth=3.0)
    #         plt.legend()
    #
    #     plt.show()
    #     inferred_test_traj.append(current_inferred_traj)
