...
def state_segmentation(x, y, psi, vx, vy, plot = False):
    var = np.zeros(D)
...
for i in range(len(train_x)):
     seg_ranges.append(state_segmentation(train_x[i], train_y[i], train_vx[i], train_vy[i], train_psi[i]))
...
def weight_gen(seg_range,x,y,vx,vy,psi,big_psi, plot = False):
    #equation 11
    list_ww = []
    for seg_idx in range(len(seg_range)):
    #for seg_idx in range(1):
...
for i in range(len(seg_ranges)):
    list_ww = weight_gen(seg_ranges[i], train_x[i], train_y[i], train_vx[i], train_vy[i], train_psi[i], big_psi)
    if train_x[i][-1] > 0:
        list_pos_ww.extend(list_ww)
    else:
...
seg_ranges_obv = []
for i in range(len(test_x)):
     seg_test_obs = state_segmentation(test_x[i], test_y[i], test_vx[i], test_vy[i], test_psi[i])
...
def reconstruct_seg(observation_data, mean_ww, sigma_ww):
