...
if __name__ == "__main__":
    ...
    for i in range(len(test_x)): # all data for testing
        ...
        inferred_traj_mp = np.dot(big_psi_ent_mean,mean_ent_ww)
        inferred_traj_mp_len= int(len(inferred_traj_mp)/D)
        # #plt.plot(trained_traj_ent[0:obs_data_len*D: D],trained_traj_ent [1:obs_data_len*D:D], 'ro' )
        # #plt.plot()
        plt.plot(obs_traj[0:obs_data_len*D: D],obs_traj[1:obs_data_len*D:D], 'ro')
        plt.plot(inferred_traj[0:inferred_traj_len*D: D], inferred_traj[1: inferred_traj_len*D: D], 'bo')
        plt.plot(inferred_traj_mp[0:inferred_traj_mp_len*D: D], inferred_traj_mp[1: inferred_traj_mp_len*D: D], 'yo')
