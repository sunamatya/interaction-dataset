...
class VisUtils:
    ...
    def draw_dist(self):
        ...
        tester = np.array(self.sim.agents[1].predicted_actions_other)
        ax2.plot(tester[:,1], label='predicted', linestyle='--')
        ax2.set_ylim([-10, 10])
        ...
        tester = np.array(self.sim.agents[1].predicted_actions_self)
        ax3.plot(tester[:,1], label='predicted', linestyle='--')
        ax3.set_ylim([-10, 10])
